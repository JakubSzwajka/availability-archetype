...
class TestResourceMixedCases:
    def setup_method(self):
        ...
        # default Monday 9-17
        self.resource.set_default_availability({0: SlotSet({TimeSlot(Time(9, 0), Time(17, 0))})})
    ...
    def test_overwrite_and_lock_combination(self):
        ...
        self.resource.set_overwrite_availability(test_date, SlotSet({TimeSlot(Time(9, 0), Time(17, 0))}))

        # lock a chunk 11-13 via API
        self.resource.lock(DateSlot(test_date, Time(11, 0), Time(13, 0)))

        assert self.resource.is_available(DateSlot(test_date, Time(10, 0), Time(11, 0))) is True
        assert self.resource.is_available(DateSlot(test_date, Time(11, 30), Time(12, 0))) is False
        assert self.resource.is_available(DateSlot(test_date, Time(16, 0), Time(17, 0))) is True
    ...
    def test_overwrite_disables_default_completely(self):
        ...
        self.resource.set_overwrite_availability(test_date, SlotSet())

        assert self.resource.is_available(DateSlot(test_date, Time(10, 0), Time(11, 0))) is False

    def test_lock_outside_overwrite_window(self):
        test_date = date(2025, 6, 25)  # Wednesday, no default

        # overwrite 9-12
        self.resource.set_overwrite_availability(test_date, SlotSet({TimeSlot(Time(9, 0), Time(12, 0))}))

        # lock 14-15 (outside overwrite)
        self.resource.lock(DateSlot(test_date, Time(14, 0), Time(15, 0)))
    ...
    def test_lock_and_unlock(self):
        ...
        booking_slot = DateSlot(test_date, Time(14, 0), Time(15, 0))
