...
class TestResourceDayOff:
    ...
    def test_unavailable_whole_day(self, start, end):
        assert self.resource.is_available(DateTimeSlot(self.day_off, start, end)) is False

    def test_other_day_still_available(self):
        other_day = date(2025, 8, 5)  # Tuesday
        assert self.resource.is_available(DateTimeSlot(other_day, Time(10, 0), Time(11, 0))) is False  # no default Tuesday

        # add default Tuesday to verify positive
        self.resource.set_default_availability({1: TimeSlotSet({TimeSlot(Time(9, 0), Time(17, 0))})})
        assert self.resource.is_available(DateTimeSlot(other_day, Time(10, 0), Time(11, 0))) is True
    ...
    def test_day_off_unlock(self):
        ...
        assert self.resource.is_available(DateTimeSlot(self.day_off, Time(10, 0), Time(11, 0))) is True
