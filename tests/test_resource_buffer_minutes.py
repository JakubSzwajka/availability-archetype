...
class TestResourceBufferMinutes:
    ...
    def test_buffer_effect(self, start: Time, end: Time, expected: bool):
    ...
    def test_back_to_back_locks_with_buffer(self):
        ...
        assert (
            self.resource.is_available(
                DateTimeSlot(self.lock_date, Time(14, 20), Time(14, 30))
            )
            is False
        )
        ...
        assert (
            self.resource.is_available(
                DateTimeSlot(self.lock_date, Time(15, 15), Time(15, 30))
            )
            is True
        )
